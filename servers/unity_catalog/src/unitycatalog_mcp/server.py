...
from .tools import list_all_tools, Content
...
def _warn_if_duplicate_tool_names(tools: list[BaseTool]):
    ...
    duplicates = [name for name, cnt in collections.Counter(tool_names).items() if cnt > 1]
    if duplicates:
        LOGGER.warning(
...
async def handle_sse(scope, receive, send):
    # this opens the SSE stream at GET /sse
